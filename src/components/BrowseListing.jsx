...
const BrowseListings = () => {
  ...
  if (!user) {
    return (
      <div className={`min-h-screen flex items-center justify-center text-center p-6 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <Toaster />
        <div className="max-w-md mx-auto">
          ...
          <p className={`mb-6 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
            You need to log in to browse roommate listings.
          </p>
          <Link
            to="/login"
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'} py-8`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Header */}
        <div className={`rounded-xl p-6 mb-8 shadow-sm ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className={`text-sm px-3 py-2 rounded-lg ${theme === 'dark' ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
    ...
    focus:outline-none focus:ring-2 focus:ring-blue-500/20`}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 px-6">
            <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredListings.map((item) => (
              <div
                key={item._id}
                className={`rounded-xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl ${theme === 'dark' ? 'bg-gray-800 hover:bg-gray-750' : 'bg-white hover:bg-gray-50'}`}
              >
                {/* Card Content */}
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="flex items-center text-sm font-medium text-pink-500">
                      ...
                      {(() => {
                        // Handle different like count formats from your backend
                        if (Array.isArray(item.likes)) return item.likes.length;
                        if (typeof item.likes === 'number') return item.likes;
                        if (item.likeCount) return item.likeCount;
                  ...
                  <div className="flex justify-between items-center">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${item.availability
                        ? theme === 'dark'
