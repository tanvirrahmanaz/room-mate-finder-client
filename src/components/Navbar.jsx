...
const Navbar = () => {
  ...
  // Close popups when clicking outside (but not on hover)
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (popupRef.current && !popupRef.current.contains(event.target)) {
        // Only close if not hovering over the popup area
        const isHoveringPopup = popupRef.current.matches(':hover');
        if (!isHoveringPopup) {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="relative inline-flex items-center cursor-pointer group p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors duration-200"
              >
              ...
              {user ? (
                ...
                >
                  ...
                  <div className={`absolute right-0  w-64 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50 transition-all duration-200 ${
                    userPopupOpen 
                    ...
                    <div className="py-2">
                      <button
                        ...
                        className="flex items-center space-x-2 w-full px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-100"
                      >
                        <LogOut size={16} />
                        <span>Sign Out</span>
                      </button>
